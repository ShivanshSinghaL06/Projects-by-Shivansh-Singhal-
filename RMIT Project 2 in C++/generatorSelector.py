...
# Code Implemented By SHIVANSH SINGHAL S3957577
    ...
    def match(self, solver: MazeSolver) -> MazeGenerator:
        ...
        @param solver: Instance of a maze solver you should generate a maze to maximum the number of cells it explores.
        ...
        if solver.m_name == "pledge":
            # Recursive backtracking solver (uniformly random)
            generator = TaskDMazeGenerator()
        elif solver.m_name == "recur":
            # Recursive backtracking solver (preference towards a certain direction)
            generator = TaskDMazeGenerator()
        elif solver.m_name == "wall_follower":
            # Wall following algorithm (right-hand)
            generator = TaskDMazeGenerator()
        elif solver.m_name == "taskC":
            # Pledge (right-hand for wall following part)
            generator = TaskDMazeGenerator()
        else:
            # Default option is to use Task D generator
            generator = TaskDMazeGenerator()
